...
const generateLoginToken = (username) => {
  return jwt.sign(
    { username },
    process.env.JWT_LOGIN_SECRET || 'login_secret_key_change_in_production',
...
const verifyAccessToken = (req, res, next) => {
  ...
  try {
    const decoded = jwt.verify(token, process.env.JWT_ACCESS_SECRET || 'access_secret_key_change_in_production');
    req.user = decoded;
    next();
  } catch (err) {
...
const verifyLoginToken = (token) => {
  try {
    return jwt.verify(token, process.env.JWT_LOGIN_SECRET || 'login_secret_key_change_in_production');
  } catch (err) {
    return null;
  }
};
